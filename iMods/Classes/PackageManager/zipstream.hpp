...
public:
	typedef std::basic_istream<Elem, Tr>& istream_reference;
    typedef std::basic_ios<Elem, Tr> base_basic_istream;
...
public:
	...
	: 
		...
		m_is_gzip(is_gzip_),
    ostream_type(std::basic_ostream<Elem,Tr>::rdbuf())
	{
		if (m_is_gzip)
			add_header();
	};
...
public:
	...
	*/
    bool check_crc() const				{	return zip_istreambase_type::get_crc() == m_gzip_crc;};
	/// return data size check
    bool check_data_size() const		{	return zip_istreambase_type::get_out_size() == m_gzip_data_size;};
