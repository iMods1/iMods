...
- (void)launchTaskWithOptions:(NSDictionary *)options {
    ...
    if (!locked) {
        [self appendTextToTextView:[NSLocalizedStringFromTableInBundle(@"Unable to lock dpkg, it's probably used by another app, such as Cydia.", nil, [self translationsBundle], nil) stringByAppendingString:@"\n"]];
    ...
    .then(^{
        ...
        if (self.status == FinishedSuccessfully) {
            [self appendTextToTextView:[@"" stringByAppendingString:[NSLocalizedStringFromTableInBundle(@"Done", nil, [self translationsBundle], nil) stringByAppendingString:@"\n"]]];
            [self.delegate installationDidFinish:self];
        } else {
            [self appendTextToTextView:[NSLocalizedStringFromTableInBundle(@"Package manager exited with error.", nil, [self translationsBundle], nil) stringByAppendingString:@"\n"]];
        }
    });
#else
#endif
