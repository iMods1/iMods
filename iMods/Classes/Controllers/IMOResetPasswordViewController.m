...
- (IBAction)didTapSubmitButton:(id)sender {
    ...
    if ([newpwd length] == 0 || [confirmNewpwd length] == 0) {
        self.errorLabel.text = NSLocalizedStringFromTableInBundle(@"Password fields cannot be empty", nil, [self translationsBundle], nil);
        return;
    }
    if (![newpwd isEqualToString:confirmNewpwd]) {
        self.errorLabel.text = NSLocalizedStringFromTableInBundle(@"Password fields don't match", nil, [self translationsBundle], nil);
        return;
    }
    ...
    .catch(^(NSError* error){
        if (error.ovc_response.HTTPResponse.statusCode == 403) {
            self.errorLabel.text = NSLocalizedStringFromTableInBundle(@"The reset password token has expired, please send a new request.", nil, [self translationsBundle], nil);
        } else {
    ...
    .then(^{
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTableInBundle(@"Password Reset", nil, [self translationsBundle], nil)
                                                        message:NSLocalizedStringFromTableInBundle(@"Your password has been reset.", nil, [self translationsBundle], nil)
                                                       delegate:self
