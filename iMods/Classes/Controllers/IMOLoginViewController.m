...
- (IBAction)loginButtonWasTapped:(UIButton *)sender {
    ...
    [manager userLogin:self.emailField.text
        ...
        if (successful) {
            [IMOKeychainManager addLoginToKeychainForUser:self.emailField.text andPassword:self.passwordField.text];
        }
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:self.emailField.text forKey:@"email"];
        [defaults setObject:[NSDate date] forKey:@"lastProperLogin"];
        ...
        // Server down... need to hack around this view
        [self.delegate loginViewControllerDidFinishLogin: self];
        [self dismissViewControllerAnimated:YES completion:^{}];
    });
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (alertView.tag == 1 && buttonIndex == alertView.firstOtherButtonIndex) {
        ...
        .catch(^(NSError* error){
            NSString* msg = [NSLocalizedStringFromTableInBundle(@"Error occurred during sending request:", nil, [self translationsBundle], nil) stringByAppendingString:[NSString stringWithFormat:@" %@", error.localizedDescription]];
        ...
        .then(^{
            UIAlertView* alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTableInBundle(@"Reset password", nil, [self translationsBundle], nil)
                                                            message:NSLocalizedStringFromTableInBundle(@"The reset password link has been sent to your email address.", nil, [self translationsBundle], nil)
                                                           delegate:nil
                                                  cancelButtonTitle:NSLocalizedStringFromTableInBundle(@"OK", nil, [self translationsBundle], nil)
                                                  otherButtonTitles:nil];
            [alert show];
